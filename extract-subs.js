...
(async () => {
  try {
    const files = await fs.readdir('Offline');
    const mkvs = files.filter((item) => /\.mkv$/.test(item));
    const tasks = [];
    for (const mkv of mkvs) {
      const { stdout: output, stderr } = await exec(`mkvinfo "Offline/${mkv}"`);
      ...
      if (output) {
        ...
        let tracks = [];
        let subtitles = [];
        while ((line = pattern1.exec(output)) != null) {
          if (line[1].indexOf('|+ Tracks') > -1) break;
        }
        let result;
        while ((line = pattern1.exec(output)) != null) {
          if (line[1].indexOf('|+') > -1) break;
